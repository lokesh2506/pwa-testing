...
- Zustand store integration (`useGlobalDataStore`)
...
4. Selection triggers `setFilters()` → `useGlobalDataStore` calls `POST /api/dashboard`
5. API returns filtered data → Components subscribe to `useGlobalDataStore`
